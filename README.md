...
App is hosted [here](https://linussgi.github.io/arma-signal-simulator/). Note: For best experience, open link on a desktop browser (not currently mobile friendly).
...
- The poles (denoted by red X's) and the zeros (denoted by blue O's) can be dragged around the complex plane, and the discrete time response updates dynamically on the right hand plot.
- Poles and zeros can be added with the `ADD PAIR` buttons. A maximum of 4 poles and 4 zeros can be added and manipulated.
- The magnitude of the frequency response of the system is shown below the unit circle plot. The ticks are marked as multiples of $\pi$ from 0 to 1.
...
$$H(z) = \frac{\theta_0 + \theta_1 z^{-1} + \theta_2 z^{-2} + \ldots + \theta_M z^{-M}}{1 + \phi_1 z^{-1} + \phi_2 z^{-2} + \ldots + \phi_N z^{-N}} = \frac{\prod \limits^{M}_{n = 1} (1 - z_n z^{-1})}{\prod \limits^{N}_{n = 1}(1 - p_n z^{-1})}$$

Where $p_n$ are the system poles and $z_m$ are the system zeros. It is these poles and zeros that are plotted on the unit circle plot on the web page. Evaluating the transfer function $H$ at $z = e^{j \omega}$ yields the frequency response. 

To enforce BIBO stability, the poles are constrained to a unit magnitude or smaller. For real valued signals, the poles and zeros always come in complex conjugates, which is why moving one pole in the complex plane will also force its conjugate to move.